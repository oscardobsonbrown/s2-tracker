...
class RepoAnalyzer:
    ...
    def _check_feature_flags(self) -> bool:
        ...
        for manifest_name in ["requirements.txt", "go.mod", "Gemfile"]:
            ...
            if content and any(
                flag_pattern in content.lower() for flag_pattern in patterns
            ):
                return True
    ...
    def _evaluate_style_validation(self) -> list[CriterionResult]:
        ...
        if not large_file:
            ...
            large_file = any(
                x in husky_pre_commit.lower()
                for x in ["large file", "1048576", "git lfs", "check for large files"]
            )
        results.append(
            self._make_result(
        ...
        for wf in workflows:
            ...
            if any(
                x in content.lower()
                for x in ["vulture", "knip", "deadcode", "dead-code"]
            ):
                dead_code = True
                break
        results.append(
        ...
        for wf in workflows:
            ...
            if any(
                x in content.lower()
                for x in ["jscpd", "pmd cpd", "sonarqube", "duplicate-code"]
            ):
                duplicate = True
                break
        results.append(
        ...
        n_plus_one = False
        # Check for N+1 detection in dependencies
        deps = (
        ...
        if any(x in deps.lower() for x in ["nplusone", "bullet", "query-analyzer"]):
            n_plus_one = True
        # Check for custom N+1 detection script
        if not n_plus_one:
            nplusone_scripts = list(
                self.repo_path.glob("**/check-n-plus-one.sh")
            ) + list(self.repo_path.glob("**/n-plus-one*.yml"))
            for wf in workflows:
                content = wf.read_text(errors="ignore")
                if "n-plus-one" in content.lower() or "n_plus_one" in content.lower():
                    n_plus_one = True
                    break
        results.append(
            self._make_result(
    ...
    def _evaluate_build_system(self) -> list[CriterionResult]:
        ...
        if any(
            ...
            heavy_deps = True
        # Check for bundle size tracking in workflows (Next.js analyze, custom scripts)
        if not heavy_deps:
            workflows = list(self.repo_path.glob(".github/workflows/*.yml")) + list(
                self.repo_path.glob(".github/workflows/*.yaml")
            )
            for wf in workflows:
                ...
                if any(
                    x in content.lower()
                    for x in ["analyze=true", "bundle-size", "bundlesize", "ANALYZE"]
                ):
                    heavy_deps = True
                    break
        results.append(
        ...
        for wf in workflows:
            ...
            if any(
                x in content.lower()
                for x in ["depcheck", "deptry", "go mod tidy", "knip", "unused-dep"]
            ):
                unused_deps = True
                break
        results.append(
        ...
        dead_flags = False
        # Check for dead feature flag detection script or workflow
        for wf in workflows:
            content = wf.read_text(errors="ignore")
            if any(
                x in content.lower()
                for x in ["feature-flag", "feature_flag", "dead.flag"]
            ):
                if any(x in content.lower() for x in ["check", "detect", "scan"]):
                    dead_flags = True
                    break
        # Also check for feature flag check script
        if not dead_flags:
            flag_scripts = list(self.repo_path.glob("**/check-feature-flags.sh"))
            if flag_scripts:
                dead_flags = True
        results.append(
            self._make_result(
        ...
        version_drift = False
        # Check for version drift detection script or workflow
        workflows = list(self.repo_path.glob(".github/workflows/*.yml")) + list(
            self.repo_path.glob(".github/workflows/*.yaml")
        )
        for wf in workflows:
            ...
            if any(
                x in content.lower()
                for x in ["version-drift", "version_drift", "workspace.*"]
            ):
                version_drift = True
                break
        # Check for custom script
        if not version_drift:
            drift_scripts = list(self.repo_path.glob("**/check-version-drift.sh"))
            if drift_scripts:
                version_drift = True
        results.append(
            self._make_result(
    ...
    def _evaluate_dev_environment(self) -> list[CriterionResult]:
        ...
        if devcontainer:
            ...
            if content and any(
                x in content.lower()
                for x in ["image", "dockercomposefile", "build", "features", "service"]
            ):
    ...
    def _evaluate_observability(self) -> list[CriterionResult]:
        ...
        logging_found = False
        deps = (
            self._workspace_manifest_contents()
            + (self._read_file("requirements.txt") or "")
        ...
        error_tracking = any(
            x in deps.lower()
            for x in ["sentry", "bugsnag", "rollbar", "honeybadger", "posthog"]
        )
    ...
    def _evaluate_security(self) -> list[CriterionResult]:
        ...
        if not log_scrub:
            log_scrub = self._file_exists(
                "packages/observability/scrub.ts", "**/*scrub*.ts", "**/*redact*.ts"
            )
        results.append(
            self._make_result(
                "log_scrubbing",
