...
import { getSurfConditions, WeatherProviderError } from "@/lib/weather";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (
      ...
      routeLogger.warn("Surf weather request had incomplete coordinates");
    ...
    const data = await getSurfConditions({
    ...
    routeLogger.info(
      ...
      "Surf weather request completed"
    );
    ...
    if (error instanceof WeatherProviderError) {
      ...
      routeLogger.warn(
        ...
        "Surf weather provider error mapped to API response"
      );
    ...
    routeLogger.error(
      ...
      "Unexpected surf weather failure"
    );
    return errorResponse(
      "PROVIDER_ERROR",
      "Surf conditions could not be loaded.",
      502
    );
  }
}
